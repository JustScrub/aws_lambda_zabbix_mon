...
Utilizing this "hack" would solve the problem. It is only required to have an S3 bucket, to which the data will not be delivered. Yet still upon failure of the transformation Lambda, a crash notification is delivered to the bucket, which wasn't a part of the original idea, but we'll call that a feature. Also, when the function fails, it is retried a number of pre-configured times with the same data, and only after all retries fail, the notification is put to the bucket. Unsurprisingly, this is the Transformation Lambda mentioned several times in this documentation. How the function processes data will be stated below.
...
Apart from the Transformation Lambda, the Discovery Lambda is created in the metric-stream.yaml SAM template as well. Operation of both will be described next.
...
Next, the Lambda runs through the extracted JSONs, now each in 1-to-1 relation with Lambda functions, and for each discovers, whether the Lambda of that object has the priority tag and whether it is flagged by Discovery Lambda as discovered in Zabbix, using the boto3 package. Names of Lambdas without the tag are stored in a set of ignored functions. After retrieving the set, the Transformation Lambda moves on to constructing a single packet containing the values for Zabbix items. First, a mapping from the AWS metric/statistic to Zabbix item name is loaded from a file. The mapping file, called metric_map.json, is silently exported during configuration by the prj_config.py script based on the configured metrics in metrics_def.py. Along with the mapping file, the python configuration is exported. The mapping file is a simple JSON object with parameters named after AWS metrics that were found in the MetricConfigs list in metrics_def.py, the value of each such parameter is an object that now has fields equal to AWS statistic names found in the MetricConfigs list. The values here are lists of Zabbix item names, to which the metric/statistic should be pushed. For example, metric_map.json could be:
...
This mapping hints the Transform Lambda to push Errors/sum values to the `errors.metrics.<suffix>[<FunctionName>]` Zabbix item, Duration/max values to `max.duration.metrics.<suffix>[<FunctionName>]` and to `count_avg.duration.metrics.<suffix>[<FunctionName>]` Zabbix items and accordingly for the min statistic of the Duration metric. The Transformation Lambda runs through the extracted JSON objects again and for each, where the Lambda is not in the ignored set, gets all the Metric/statistic pairs from the mapping file and for each Zabbix item in the pair's list, constructs an instance of the `SenderData` class of the zappix package, containing all the required information for Zabbix: the host name (taken from ZBX_SUFFIX in config), item name and value (determined by the mapping file) and also second and nanosecond timestamps generated from the JSON's timestamp provided by Metric Stream (in miliseconds, so two some conversions are made). Yes, the actual code iterates through not three, but four nested for loops. Inside python's list comprehention syntax. With an 'if' clause. Look at it. It's beautiful. The function `zbx_mass_item_packet()` handles this and returns a list of the `SenderData` objects.
...
First, change your metrics_def.py with your new set of metrics (including the old ones you wish to "keep") and then re-run prj_config.py. If you wish to keep one of the two sets (file configs or SAM params JSON) unchanged, press CTRL+C while prompted to fill its values.
