...
def update_priority(zab_addr,function_name,priority:LambdaPriority):
    ...
    auto_discover([(function_name,priority.value)],(zab_addr[0],10051),ZBX_SUFFIX)


if __name__=="__main__":

    if len(sys.argv) < 3:
        print("usage: python3 -m scripts.lambda_update_priority <function_name> <priority> [<frontend_host>=localhost] [<frontend_port>=80] [<server_port>=10051]")
        ...
        print("<server_port>: port of the Zabbix Server, at same IP/DNS name as Frontend")
        exit(1)
